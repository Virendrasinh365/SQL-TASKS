...
-- FULL OUTER JOIN: List all books and borrowers including books not borrowed and members with no borrows
-- (Note: Many SQL dialects like MySQL do not support FULL OUTER JOIN; this is standard SQL syntax or use UNION of LEFT and RIGHT JOIN)
SELECT B.Title, M.Name AS BorrowerName
FROM Books B
FULL OUTER JOIN Borrows BR ON B.BookID = BR.BookID
FULL OUTER JOIN Members M ON BR.MemberID = M.MemberID;

-- Self-join example: Find pairs of authors from the same country
SELECT A1.AuthorName AS Author1, A2.AuthorName AS Author2, A1.Country
FROM Authors A1
INNER JOIN Authors A2 ON A1.Country = A2.Country AND A1.AuthorID <> A2.AuthorID;

-- Join multiple tables to show detailed borrowing info (borrow date, member, book, author)
SELECT BR.BorrowDate, M.Name AS MemberName, B.Title AS BookTitle, A.AuthorName
FROM Borrows BR
JOIN Members M ON BR.MemberID = M.MemberID
JOIN Books B ON BR.BookID = B.BookID
JOIN Authors A ON B.AuthorID = A.AuthorID;
...
-- Subquery in WHERE: Find authors who have written more than 1 book
SELECT AuthorName
FROM Authors
WHERE AuthorID IN (
    SELECT AuthorID
...
-- Scalar subquery in SELECT: Show books with the count of borrowers for each book
SELECT Title,
    (SELECT COUNT(*)
     FROM Borrows BR
     WHERE BR.BookID = B.BookID) AS BorrowerCount
FROM Books B;

-- Correlated subquery in WHERE: Find members who borrowed a book on or after 2025-11-01
SELECT Name
FROM Members M
WHERE EXISTS (
    SELECT 1
    ...
    WHERE BR.MemberID = M.MemberID
      AND BR.BorrowDate >= '2025-11-01'
);

-- Subquery with IN clause: Find books borrowed by member with ID 1
SELECT Title
FROM Books
WHERE BookID IN (
    SELECT BookID
...
-- Subquery in FROM clause (derived table): Get average copies available grouped by author, then list authors with average copies > 2
SELECT AuthorName, AvgCopies
FROM (
    SELECT A.AuthorName, AVG(B.CopiesAvailable) AS AvgCopies
    FROM Books B
    JOIN Authors A ON B.AuthorID = A.AuthorID
...
-- Nested subqueries: Find members who borrowed books published by 'Bloomsbury'
SELECT Name
FROM Members
WHERE MemberID IN (
    SELECT MemberID
